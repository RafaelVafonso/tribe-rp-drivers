 ...
 * including methods for setting the duty cycle, frequency, alignment, and enabling/disabling the
 * PWM signal.
 */
class PWMout {
public:
     ...
     */
    PWMout(uint pin, float freq_hz = 1000.0f, float duty = 0.0f, bool enabled = true,
     ...
     * This method internally converts the input value to a [0.0, 1.0] range before setting the
     * duty.
     *
     * @param duty The duty cycle as a percentage (0.0 to 100.0).
     */
    void set_duty(float duty) {
        duty_ = clamp01_(duty / 100.0f); /**< Convert the duty cycle percentage (0.0 to 100.0) to
     ...
     */
    uint16_t level() const { return (uint16_t) std::min<uint32_t>(duty_to_level_(duty_), wrap_); }
     ...
     *
     * This method inverts the PWM output, which means that when the duty cycle is high, the output
