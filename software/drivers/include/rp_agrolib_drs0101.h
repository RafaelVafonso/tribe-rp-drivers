/*
 * This file includes code from the HerkulexServo Library
 * https://github.com/cesarvandevelde/HerkulexServo?tab=MIT-1-ov-file,
 * originally licensed under the MIT License.
 *
 * Modifications made by INESC TEC on December, 2023:
 * - Changed the communication interface library to make the code usefull for RP2040 integration
 */
...
enum class HerkulexScheduleState : uint8_t { None = 0, IndividualMove, SynchronizedMove };
...
public:
    ...
    void sendPacket(uint8_t id, HerkulexCommand cmd, uint8_t *data = nullptr, uint8_t data_len = 0);
    bool sendPacketAndReadResponse(HerkulexPacket &resp, uint8_t id, HerkulexCommand cmd,
                                   uint8_t *data = nullptr, uint8_t data_len = 0);
    void update();
    ...
    HerkulexServoBus(uart_inst_t *_uart_id) : uart_id(_uart_id) {
...
protected:
    ...
    uint8_t m_rx_buffer[HERKULEX_SERIAL_RX_BUFFER];
    uint8_t m_rx_buffer_index = 0;
    clock_t m_last_serial = 0;
